...
- 连接器提供 `tool_call_id` 时按 id 精确配对，**且被配对的 tool_call 必须位于该条 result 之前**（B03，#131：拼接/压缩后的日志会留下「调用已不在、结果还在」的孤儿结果；若后面另有调用复用了同一 id，旧实现把这条普通结果绑到那个之后的调用上，于是被当该调用的命中清掉）。同一 id 被两个不同 `tool_call` 复用时，该 id 一律不配对（宁漏勿误）。
...
| 连接器 | 承载正文的 JSON 路径 |
|---|---|
| `claude_code` | `message.content[*].content`、`message.content[*].text`、`toolUseResult.file.content`；`historical_raw_json` 封装（`sqlite.rs:2046`）内的同名路径同样替换（先解包字符串内 JSON，按同表路径替换，再序列化写回）。另有 `message.content[*].thinking`（`thinking` 块，独立成 `role='reasoning'` 行）与顶层 `content`（`type=system,subtype=away_summary` 事件，独立成 `role='assistant'` 行）——两者是 T1b 探针为对齐 DB `(idx,role)` 序列而发现的正文承载点，供 T2 参考，不在 spec 原始四类锚点范围内，本身不参与排除判定。**顶层字符串型 `toolUseResult`**（R2-B4，任务书 #119a）：区别于上面对象形的 `.file.content` 子路径——同一字段名在不同工具的输出上是两种互斥的形状，对象形携带 `filePath`/`type` 等非正文元数据（子路径精确清除，兄弟字段不动），字符串形是正文本身（整体替换）——**整体替换只在能证明该字符串属于本次被清的块时发生**（B04，#131：字符串等于、或在其长度门内包含，本次被清块之一的清空前正文才清；承载别的块的正文时保留，否则同一事件两个结果块、只排除一个时会留下被排除正文的副本）。**数组型 `toolUseResult`**（N-R7arr，#131 T6-c）：MCP 类工具结果记为 `[{"type":"text","text":…}]`，同一条归属证明逐元素施加于元素的 `text`（元素下标与块下标无关，由归属证明而非位置决定是否可清）；缺这一路时被排除正文会原样留在 `/toolUseResult[0]/text`（T6-b 候选库实测 595 B，会话 `…cc-workspace…` idx=48 cass_recall）。**归属正文的来源与拼接规则**（R9-B03，#132）：归属证明比对的是「被清块的清空前正文，按连接器投影规则（`utils.rs::extract_content_part` + `flatten_content`）得到」——目标块的 `content`/`text` 为**数组**时（`tool_result.content = [{"type":"text","text":…}]`）其投影（非空部分以单个 `\n` 拼接）就是该块的正文，只读字符串会漏掉整个数组；同理数组型 `toolUseResult` 是**一个**投影值，当它的元素是同一正文的**碎片**、拼接后等于归属正文时，整个数组都是该正文的副本，逐元素全部替换（只按单元素相等判定会漏掉碎片）。反向边界：拼接结果不等于任何归属正文时不整体替换，退回逐元素归属证明（各元素 `text` 分别比对，仅被证明归属的元素被清）。冻结库实测（`context_file_read`/`cass_recall` 两类，claude_code）：1,533/1,682 抽样 + 25/28 分别命中字符串形且与 `message.content` 字节相同 |
...
`rule_version 1` = PR6 合入时的语义，含 R4 的位置约束（B03）与 R7 顶层 `toolUseResult` 的归属证明（B04 的字符串形、N-R7arr 的数组形逐元素）；PR6 之前的 v1 快照未部署过，不构成升版事件（控制面 2026-09-12 裁）。
