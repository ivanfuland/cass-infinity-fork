...
// W2-6 exec39: `seed_active_rebuild_runtime` removed here -- its only
// caller in this file, `status_json_surfaces_runtime_queue_and_byte_budget_headroom`,
// was itself deleted (see the disposition note above
// `status_json_surfaces_quarantine_gc_summary` below). e2e_health.rs
// keeps its own copy, rewritten to plant a real DB marker instead of the
// retired checkpoint file, because 3 other tests in that file still use
// it. See w2-6-deleted-tests.md for the full disposition record.
...
// W2-6 exec39: `status_json_surfaces_runtime_queue_and_byte_budget_headroom`
// removed here -- its entire assertion surface (`rebuild.pipeline.runtime`
...
// carries no progress counters"). See w2-6-deleted-tests.md for the full
// disposition record.
