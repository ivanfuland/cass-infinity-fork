...
pub(crate) trait StorageBackend {
    ...
    fn begin(&mut self, mode: TxMode) -> Result<(), StorageError>;
...
fn query_row_map_impl<T>(
    ...
    match count {
        0 => Err(StorageError::Other { code: None, detail: "query returned no rows".into() }),
...
impl Conn {
    ...
    pub fn transaction(&mut self) -> Result<Tx<'_>, StorageError> {
        self.inner.begin(TxMode::Deferred)?;
        Ok(Tx { backend: self.inner.as_mut(), finalized: false })
    }
...
pub struct Tx<'c> {
    backend: &'c mut dyn StorageBackend,
    finalized: bool,
}
