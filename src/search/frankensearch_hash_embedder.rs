//! W3-5 verbatim restore of `frankensearch-embed`'s FNV-1a hash embedder
//! algorithm, now that the `frankensearch` Cargo dependency itself is
//! retired. `crate::search::hash_embedder::HashEmbedder` delegates its core
//! hashing/projection logic to `HashEmbedder` below (see that module's
//! `delegate` field) -- this is not part of the two-tier/progressive
//! retirement: `indexer::semantic::embed_messages` (the `db_vector_catchup`
//! embed step) still uses the hash embedder as a fallback path.
//!
//! Source: `frankensearch` git rev `2cad158f4468ece7076e3fe529c8e5c20b2e020e`
//! (<https://github.com/Dicklesworthstone/frankensearch>),
//! `crates/frankensearch-embed/src/hash_embedder.rs`, copied verbatim below.

use asupersync::Cx;

use super::frankensearch_types::{Embedder, ModelCategory, SearchFuture, l2_normalize};
...
mod tests {
    ...
    fn jl_random_pairs_approximately_orthogonal() {
        use super::super::frankensearch_types::cosine_similarity;
